...
非当前账号显示**带时间戳的上次用量**，也可点击账号旁的刷新按钮单独查询 Codex 额度（实验功能，不切换登录）。不会为了刷新每张卡片而后台来回切换账号或独立刷新它们的 token。服务端撤销、过期、其他设备刷新等情况仍可能要求重新登录。
...
账号列表直接显示每个账号缓存的 7d 剩余比例、上次查询时间和本地重置日期。Antigravity 按当前选择的 Gemini／其他模型分组显示。重启应用后仍保留；超过重置时间会显示「待刷新」，不会自动将余额改成 100%。尚未查到的账号显示「尚未查询」或「重置时间未提供」。其他设备产生的新用量需要重新向服务端查询，缓存本身不会同步。

### 单独刷新 Codex 账号（实验功能）

菜单和设置中的每个 Codex 账号旁都有刷新按钮。也可运行 `codex-switch usage "账号名称或ID"`，随后用 `codex-switch status --json` 查看结果；返回 working 表示仍在查询。查询不会切换当前账号，失败时保留旧缓存。

仅使用已保存且仍有效的登录进行一次官方查询，不自动续期。登录过期时需重新登录并保存；不会为另一台设备轮换登录凭证。此能力依赖官方实验接口，CLI 不支持或服务拒绝时会明确失败。agy 暂不支持指定非当前账号刷新。
